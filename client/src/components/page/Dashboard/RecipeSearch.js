...
class Dashboard extends Component {
  ...
  handleResultSelect = (prop, result) => {
    ...
    if (this.props.currUser[prop].includes(newItem)) {
      console.log(newItem, 'already present in', prop);
      return;
    }
    if (prop === 'pantry' && this.props.currUser.allergies.includes(newItem)) {
    ...
    this.props.currUser[prop].push(newItem);
    this.setState({
      [prop]: '',
    ...
    axios
      .patch(`/api/users/${this.props.currUser.id}`, {
        [prop]: newItem
      })
      .then(() => {
    ...
    axios
      .get(
        `/recipeAPI/recipes/complexRecipe/?query=${this.state.desiredMeal}` +
          ...
          `&diet=${this.props.currUser.dietary_restrictions.join('%2C+')}` +
          `&includeIngredients=${this.state.includeAdditionalIngredients
            ...
            // .concat(this.props.currUser.pantry)
            .join('%2C+')}` +
          `&excludeIngredients=${this.state.excludeAdditionalIngredients.join('%2C+')}` +
          `&intolerances=${
            this.state.filterAllergies ? this.props.currUser.allergies.join('%2C+') : ''
          }`
      )
      .then(res => {
        console.log(res);
                            ...
                            `/recipeAPI/recipes/complexRecipe/?query=${this.state.desiredMeal}` +
                              ...
                              `&intolerances=${
                                this.state.filterAllergies
                                  ? this.props.currUser.allergies.join('%2C+')
                                  : ''
                              }`}
                            '
                          </Modal.Header>
